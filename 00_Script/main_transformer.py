...
class DFRTransformer(CredentialTransformer):
    def transform(self) -> Dict[str, Any]:
        ...
        services = self.component.get('services', [])
        ...
        render_template = next((param.get('renderTemplate', []) for service_group in services for param in service_group.get('parameters', []) if 'renderTemplate' in param), [])
...
if __name__ == "__main__":
    ...
    print(current_dir)
    processor = AppConfigProcessor(current_dir.parent / "01_Data/app-config" / "app-config.json")
    output = processor.process()

    output_path = current_dir.parent / "01_Data/app-config" / "transformed-app-config.json"
