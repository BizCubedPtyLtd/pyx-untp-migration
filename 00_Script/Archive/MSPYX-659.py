# 1. Version-Aware Migration Handler

def migrate_dfr(json_data, from_version="0.5.0", to_version="0.6.0"):
    if from_version == "0.5.0" and to_version == "0.6.0":
        return migrate_dfr_v050_to_v060(json_data)
    raise NotImplementedError(f"No migration path from {from_version} to {to_version}")
# 2. Specific Transformer Function (v0.5.0 → v0.6.0)

def migrate_dfr_v050_to_v060(dfr_component):
    data = dfr_component["props"]["data"]
    ...
    # Update credentialSubject structure
    facility = data["credentialSubject"]
    facility_record = {
        "type": ["FacilityRecord"],
        "facility": {
            ...
            "countryOfOperation": facility.get("countryOfOperation"),
            "address": facility.get("address"),
            "locationInformation": facility.get("locationInformation"),
            "conformityClaim": facility.get("conformityClaim", [])
        }
    }

    # Move and simplify identifiers
    if "operatedByParty" in facility:
        operated_by = facility["operatedByParty"]
        facility_record["facility"]["operatedByParty"] = {
            "id": operated_by.get("id"),
            "name": operated_by.get("name"),
            "registeredId": operated_by.get("registeredId")
        }

    if "otherIdentifier" in facility:
        facility_record["facility"]["facilityAlsoKnownAs"] = [
            {
                "id": ident.get("id"),
                "name": ident.get("name"),
                "registeredId": ident.get("registeredId")
            } for ident in facility["otherIdentifier"]
        ]

    data["credentialSubject"] = facility_record
    data["type"] = ["DigitalFacilityRecord", "VerifiableCredential"]

    # Update schema URL
    dfr_component["props"]["schema"]["url"] = (
        "https://jargon.sh/user/unece/DigitalFacilityRecord/v/0.6.0/"
        "artefacts/jsonSchemas/FacilityRecord.json?class=FacilityRecord"
    )

    # Update renderTemplate if present
    for svc in dfr_component.get("services", []):
        if "parameters" in svc:
            for param in svc["parameters"]:
                if "digitalFacilityRecord" in param:
                    param["digitalFacilityRecord"]["context"] = [
                        "https://test.uncefact.org/vocabulary/untp/dfr/0.6.0/"
                    ]
                    param["digitalFacilityRecord"]["renderTemplate"] = [
                        {
                            "type": "WebRenderingTemplate2022",
                            "template": "{v0.6.0-template}..."
                        }
                    ]
                if "vckit" in param and "issuer" in param["vckit"]:
                    issuer = param["vckit"]["issuer"]
                        ...
                        issuer["issuerAlsoKnownAs"] = [
                            {
                                ...
                                "name": ident.get("name"),
                                "registeredId": ident.get("registeredId")
                            } for ident in issuer["otherIdentifier"]
                        ]
                        del issuer["otherIdentifier"]

    return dfr_component
# 🔁 Detecting and Applying Migration for DFRs in app-config.json

def migrate_app_config(config_json, from_version="0.5.0", to_version="0.6.0"):
    migrated_config = []
    for section in config_json:
        if section["id"] == "produce_product":
            for comp in section.get("components", []):
                if comp["type"] == "EntryData" and "DigitalFacilityRecord" in comp["props"]["data"]["type"]:
                    comp = migrate_dfr(comp, from_version, to_version)
        migrated_config.append(section)
    return migrated_config
# 📦 Future Proofing
# To scale to future migrations:

# Add migrate_dfr_v060_to_v070(...) and extend the migrate_dfr() dispatcher.

# Add validation checkers to ensure backward compatibility before applying.
...
# Migration function provided earlier
def migrate_dfr_v050_to_v060(dfr_component):
    ...
    # Update credentialSubject structure and fills in the new format based on the existing data: 2. Credential Subject Structure
    facility = data["credentialSubject"]
    print(facility)
    facility_record = {
        "type": ["FacilityRecord"],
    ...
    print(facility_record)
    
    
    # Update renderTemplate and context in services
    for svc in dfr_component.get("services", []):
        for param in svc.get("parameters", []):
            if "digitalFacilityRecord" in param:
                param["digitalFacilityRecord"]["context"] = [
                    "https://test.uncefact.org/vocabulary/untp/dfr/0.6.0/"
                ]
                param["digitalFacilityRecord"]["renderTemplate"] = [
                    {
                        "type": "WebRenderingTemplate2022",
                        "template": "{0.6.0 render template}"                    }
                ]
            if "vckit" in param and "issuer" in param["vckit"]:
                issuer = param["vckit"]["issuer"]
                if "otherIdentifier" in issuer:
    ...
    # Update schema URL
    dfr_component["components"][0]["props"]["schema"]["url"] = (
        "https://jargon.sh/user/unece/DigitalFacilityRecord/v/0.6.0/"
        "artefacts/jsonSchemas/FacilityRecord.json?class=FacilityRecord"
    )

    return dfr_component, facility_record, facility

# Apply migration
migrated_dfr = migrate_dfr_v050_to_v060(dfr_component)

# Output transformed result
migrated_dfr_path = r"C:\Users\karinaliauw\Downloads\MSPYX-728 - Upgrade\python\DFR_migrated_v060.json"
...
######################################
######## TESTING ####################
## Python script per migration guide: https://uncefact.github.io/tests-untp/docs/next/migration-guides/v0.6.0/#digital-facility-record-dfr
#####################################


#### function to rename a string and keep the position within the array (unlike pop, that renames the key but adds the location at the end)
def pop_and_replace_key(d, old_key, new_key):
    """
    Works like dict.pop(), but renames the key in-place
    while keeping its original position in the dict.
    ...
    """
    if old_key not in d:
        return None  # or raise KeyError like dict.pop
...
def migrate_dfr_v050_to_v060(dfr_component, hbs_dfr_file_path):
    data = dfr_component["components"][0]["props"]["data"]
    #print(data)
    ####### Data model changes ##########
    #####################################
    ...
    if "otherIdentifier" in issuer:
        ...
        #issuer["issuerAlsoKnownAs"] = issuer.pop("otherIdentifier")
    ...
    if "otherIdentifier" in facility_new: #change otherIdentifier in credentialSubject TO facilityAlsoKnownAs
        #print('inside')
        #facility_new["facilityAlsoKnownAs"] = facility_new.pop("otherIdentifier")
        x = pop_and_replace_key(facility_new,"otherIdentifier", "facilityAlsoKnownAs")
        #print(x)
        # print(facility_new)
        facility_new["facilityAlsoKnownAs"] = x
    ...
    #dfr = dfr_component["services"][0]["parameters"]["digitalFacilityRecord"]
    dfr = next(iter(dfr_component.get('services', [])), {}).get('parameters', [{}])[0].get('digitalFacilityRecord')
    #print(dfr_component["services"]["parameters"])
    ...
    with open(hbs_dfr_file_path, 'r', encoding='utf-8') as file:
        hbs_template = file.read()
    ...
    ### UPDATE: CHANGE THE RENDER TEMPLATE SO ITS BEING READFROM FILE
    ...
    if "otherIdentifier" in vckit_issuer:
        ...
        #print(x)
        # print(facility_new)
        facility_new["issuerAlsoKnownAs"] = x
...
hbs_dfr_file_path = r"C:\Users\karinaliauw\Downloads\MSPYX-728 - Upgrade\python\v0.6.0\digital_facility_record\template.hbs"
# Apply migration
migrated_dfr = migrate_dfr_v050_to_v060(dfr_component, hbs_dfr_file_path)




migrated_dfr_path = r"C:\Users\karinaliauw\Downloads\MSPYX-728 - Upgrade\python\DFR_migrated_v060_test_v2.json"
with open(migrated_dfr_path, "w") as f:
    json.dump(migrated_dfr, f, indent=2)
